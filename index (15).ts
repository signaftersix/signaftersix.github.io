...
async function sendReminderToMailer(
  ...
  const { data: managementLink } = await service
    .from("request_management_links")
    .select("management_token")
    .eq("request_id", appointment.id)
    .maybeSingle();
  const token = String(managementLink?.management_token || "");
  const payload = {
    reminderType,

    manageUrl: token
      ? `https://signaftersix.github.io/manage-request.html?request=${encodeURIComponent(appointment.id)}&token=${encodeURIComponent(token)}`
      : "",
