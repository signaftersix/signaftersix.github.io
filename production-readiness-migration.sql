...
comment on table public.request_management_links is
  'Service-role-only tokens used to include the appointment-management link in every customer email.';
...
drop policy if exists "authenticated users read own admin membership" on public.admin_users;
