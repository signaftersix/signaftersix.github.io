...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // is_admin() validates both membership and the JWT's AAL2 claim. This is
    // the same authorization path used by admin-action; server clients created
    // from an Authorization header do not carry a browser session for
    // auth.mfa.getAuthenticatorAssuranceLevel().
    const { data: isAdmin, error: adminError } = await client.rpc("is_admin");
    if (adminError || isAdmin !== true) return json({ error: "Admin authorization and MFA are required." }, 403);
    ...
    if (action === "delete_availability_block") {
      const blockId = String(body.blockId || "");
      if (!blockId) return json({ error: "Missing availability block." }, 400);
      const { error } = await service.from("availability_blocks").delete().eq("id", blockId).eq("source", "manual");
      if (error) throw error;
      await audit(service, user.id, null, "availability_block_deleted", { block_id: blockId });
    ...
    if (action === "delete_request") {
      ...
      if (paid && body.confirmPaidDeletion !== true) return json({ error: "Confirm that any required refund has already been handled before deleting this paid request." }, 409);
      await call(url, serviceKey, "calendar-sync", { requestId, event: "release" });
      const { data: documents, error: documentError } = await service.from("request_documents").select("storage_path").eq("request_id", requestId);
      if (documentError) throw documentError;
      const storagePaths = (documents || []).map((item: any) => item.storage_path).filter(Boolean);
      if (storagePaths.length) {
        const { error: storageError } = await service.storage.from("notary-documents").remove(storagePaths);
        if (storageError) throw storageError;
      }
      await audit(service, user.id, requestId, "request_deleted", { former_status: record.status, payment_status: record.payment_status, document_count: storagePaths.length, customer_reference: String(record.id).slice(0, 8) });
      const { error: deleteError } = await service.from("appointment_requests").delete().eq("id", requestId);
      if (deleteError) throw deleteError;
      return json({ ok: true, message: "Request and its stored documents were permanently deleted." });
    }
